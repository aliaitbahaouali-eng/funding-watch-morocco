-- ============================================================
-- Funding Watch Morocco — Migration v6 (hotfix matching #2)
--
-- Bug corrigé : après v5 (qui a fixé donor_name), la fonction levait
--   ERROR: column reference "title" is ambiguous
-- car la RETURNS TABLE déclare title/donor_name/deadline/morocco_eligible
-- ET le CTE final (cand / opp_match) expose ces mêmes noms — plpgsql ne
-- peut pas trancher entre la colonne de sortie et celle du CTE.
--
-- Fix : qualifier explicitement les références dans les SELECT finaux
-- avec l'alias du CTE (cand.* / opp_match.*).
--
-- À exécuter dans Supabase SQL Editor APRÈS migration_v5.sql.
-- ============================================================
...
begin
  ...
  from cand
  order by final_score desc, cand.deadline asc nulls last
  limit p_limit;
end;
$$;

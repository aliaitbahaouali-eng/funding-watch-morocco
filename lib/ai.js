  ...
  try {
    const { text: raw, usage } = await callClaude({
      ...
      user:
        ...
        `  "suggested_themes": ["slug1","slug2"]\n` +
        `}\n\n` +
        `Slugs autorisés pour "suggested_themes" (1-6 max) :\n` +
        `  femmes, jeunes, climat, education, sante, rural, ess, culture,\n` +
        `  digital, droits-humains, migration, innovation,\n` +
        `  droits-minorites, monoparentalite, sante-mentale, plaidoyer,\n` +
        `  justice-acces-droit, medias-liberte-expression, personnes-agees,\n` +
        `  enfance-protection, handicap.\n\n` +
    ...
    if (Array.isArray(parsed.suggested_themes)) {
      // Sprint 4O — liste étendue : derivée de THEME_KEYWORDS pour rester en sync
      const allowed = new Set(Object.keys(THEME_KEYWORDS));
      data.suggested_themes = parsed.suggested_themes.filter((t) => allowed.has(t)).slice(0, 6);
    }
    return { ok: true, data, usage, model: MODEL };
  } catch (e) {

...
export function tplOnboardingDay3({ fullName, orgName, missingFields = [], unsubscribeToken }) {
  ...
  const missingList = missingFields.length > 0
    ...
    : '';
  return {
    subject: `${firstName || 'Bonjour'}, votre profil est presque pret`,
    htmlContent: wrap({
      title: 'Votre profil merite 3 minutes de plus',
      eyebrow: 'J+3 - Activation',
      unsubscribeToken,
      body: `
        <p style="margin:0 0 16px 0">${greeting},</p>
        <p style="margin:0 0 20px 0">Vous avez cree votre compte sur Funding Watch il y a 3 jours - bravo pour cette premiere etape.</p>
        <p style="margin:0 0 12px 0">Pour que le matching soit vraiment pertinent, il manque encore quelques infos cles sur <b style="color:${COLORS.ink}">${orgName || 'votre organisation'}</b> :</p>
        ${missingList}
        <p style="margin:20px 0;color:${COLORS.inkMuted}">Ca prend 3 minutes max et ca transforme la qualite des opportunites proposees (jusqu'a <b>3x plus de precision</b>).</p>
        ${cta(profileUrl, 'Completer mon profil')}
        <p style="margin:24px 0 0 0;font-size:13px;color:${COLORS.inkLight};line-height:1.6">
          Astuce : uploadez le PDF de presentation de votre association dans la section Profil > Documents. L'IA l'analyse automatiquement.
        </p>
      `,
    })
  };
...
/**
 * Sprint 4P — Onboarding J+5 : decouverte AI Cowriter.
 */
export function tplOnboardingDay5({ fullName, orgName, savedCount = 0, unsubscribeToken }) {
  ...
  return {
    ...
    htmlContent: wrap({
      ...
      body: `
        <p style="margin:0 0 16px 0">${greeting},</p>
        <p style="margin:0 0 20px 0">Vous avez sauvegarde <b style="color:${COLORS.ink}">${savedCount} opportunite${s}</b> cette semaine - top.</p>
        <p style="margin:0 0 20px 0">Pour chacune, vous pouvez generer en <b>30 secondes</b> un brouillon de resume executif adapte a <b style="color:${COLORS.ink}">${orgName || 'votre asso'}</b>.</p>
        <div style="margin:24px 0;padding:20px;border-radius:14px;background:${COLORS.bg};border:1px solid ${COLORS.border}">
          <div style="font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:${COLORS.brand};margin-bottom:12px">Comment ca marche</div>
          <p style="margin:6px 0;font-size:14px"><b style="color:${COLORS.ink}">1.</b> Sur une opp sauvegardee, cliquez <b>"AI Cowriter"</b></p>
          <p style="margin:6px 0;font-size:14px"><b style="color:${COLORS.ink}">2.</b> Lisez le brouillon (30 sec)</p>
          <p style="margin:6px 0;font-size:14px"><b style="color:${COLORS.ink}">3.</b> Adaptez-le a votre voix</p>
        </div>
        <p style="margin:0 0 20px 0"><b>Gain de temps : 1 a 2 heures par candidature.</b></p>
        ${cta(savedUrl, 'Voir mes opportunites sauvegardees')}
      `,
    })
  };
}
...
/**
 * Sprint 4P — Onboarding J+7 : recap premiere semaine + opps urgentes.
 */
export function tplOnboardingDay7({ fullName, orgName, stats = {}, urgentOpps = [], topThemes = [], unsubscribeToken }) {
  ...
  const urgentList = urgentOpps.slice(0, 3).map((o) => {
    ...
    const days = daysUntil(o.deadline);
    return `<div style="border:1px solid ${COLORS.border};border-radius:12px;padding:14px;margin:0 0 10px 0;background:${COLORS.white}">
      <div style="font-size:11px;font-weight:800;letter-spacing:.10em;text-transform:uppercase;color:${COLORS.brand};margin-bottom:6px">${o.donor_name || 'Bailleur'} ${days !== null ? '- ' + urgencyBadge(days) : ''}</div>
      <a href="${url}" style="color:${COLORS.ink};text-decoration:none;font-weight:700;font-size:14px;line-height:1.4">${o.title}</a>
    </div>`;
  }).join('') || `<p style="color:${COLORS.inkMuted};font-style:italic;margin:0">Aucune deadline urgente cette semaine.</p>`;
  ...
  ).join('') || `<span style="color:${COLORS.inkMuted};font-style:italic;font-size:13px">Completez votre profil pour activer la detection des thematiques.</span>`;

  return {
    subject: `Votre premiere semaine sur Funding Watch - recap`,
    htmlContent: wrap({
      ...
      body: `
        <p style="margin:0 0 16px 0">${greeting},</p>
        <p style="margin:0 0 20px 0">Petit recap de votre premiere semaine sur Funding Watch :</p>
        <div style="margin:24px 0;padding:20px;border-radius:14px;background:${COLORS.bg};border:1px solid ${COLORS.border}">
          <div style="font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:${COLORS.brand};margin-bottom:14px">Votre activite</div>
          <p style="margin:8px 0;font-size:14px"><b style="color:${COLORS.brand}">${stats.saved || 0}</b> opportunites sauvegardees</p>
          <p style="margin:8px 0;font-size:14px"><b style="color:${COLORS.brand}">${stats.applications || 0}</b> candidatures demarrees</p>
          <p style="margin:8px 0;font-size:14px"><b style="color:${COLORS.brand}">${stats.alerts || 0}</b> alertes email recues</p>
        </div>
        <div style="margin:24px 0">
          <div style="font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:${COLORS.brand};margin-bottom:10px">Vos top thematiques</div>
          ${themesPills}
        </div>
        <div style="margin:24px 0">
...
export function tplOnboardingDay14NPS({ fullName, unsubscribeToken }) {
  ...
  return {
    ...
    htmlContent: wrap({
      title: 'Comment ca se passe pour vous ?',
      eyebrow: 'J+14 - Votre avis',
      unsubscribeToken,
      body: `
        <p style="margin:0 0 16px 0">${greeting},</p>
        <p style="margin:0 0 20px 0">Cela fait 2 semaines que vous utilisez Funding Watch Morocco. J'aimerais savoir ce que vous en pensez - vraiment honnetement.</p>
        <p style="margin:24px 0 8px 0;font-weight:800;color:${COLORS.ink};font-size:15px">Sur une echelle de 0 a 10, a quel point recommanderiez-vous Funding Watch a un(e) collegue qui dirige une association marocaine ?</p>
        ...
        <p style="text-align:center;font-size:11px;color:${COLORS.inkLight};margin:6px 0 0 0">0 = Pas du tout | 10 = Absolument</p>
        <p style="margin:32px 0 0 0;color:${COLORS.inkMuted}">Vos retours guident directement les 2 prochaines features que je vais coder.</p>
...
// =================================================================
// Sprint 4P — Co-submission matchmaking
// =================================================================

/**
 * Email envoyé à l'asso cible quand une autre asso demande une mise en
 * relation pour co-soumettre sur une opportunité. Le Reply-To pointe sur
 * l'asso requester pour que la réponse soit directe.
 */
export function tplCoSubmissionRequest({ requesterOrgName, targetOrgName, opp, message, requesterContact }) {
  const oppUrl = `${APP_URL()}/opportunities/${opp.id}`;
  return {
    subject: `🤝 Co-soumission proposée — ${opp.title}`,
    htmlContent: wrap({
      title: `${requesterOrgName} propose une co-soumission`,
      eyebrow: '🤝 Mise en relation · Funding Watch',
      body: `
        <p style="margin:0 0 16px 0">Bonjour ${targetOrgName ? `<b style="color:${COLORS.ink}">${targetOrgName}</b>` : ''},</p>
        <p style="margin:0 0 20px 0">
          <b style="color:${COLORS.ink}">${requesterOrgName}</b> a vu que tu as marqué cet appel à projet comme une candidature potentielle, et propose de <b>candidater ensemble en consortium</b>.
        </p>

        <div style="border:1px solid ${COLORS.border};border-radius:16px;padding:20px;margin:20px 0;background:${COLORS.bg}">
          <div style="font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:${COLORS.brand};margin-bottom:8px">
            ${opp.donor_name || 'Bailleur'}${opp.deadline ? ` · ⏰ ${new Date(opp.deadline).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}` : ''}
          </div>
          <h2 style="margin:0 0 8px 0;font-size:17px;font-weight:800;color:${COLORS.ink};line-height:1.35">
            <a href="${oppUrl}" style="color:${COLORS.ink};text-decoration:none">${opp.title}</a>
          </h2>
        </div>

        <h3 style="margin:24px 0 8px 0;font-size:14px;font-weight:800;color:${COLORS.ink}">Le message de ${requesterOrgName}</h3>
        <blockquote style="border-left:3px solid ${COLORS.brand};padding:8px 16px;margin:8px 0;color:${COLORS.inkMuted};background:${COLORS.bg};border-radius:6px;font-size:14px;line-height:1.6">
          ${message.replace(/\n/g, '<br>')}
        </blockquote>

        ${requesterContact ? `
        <p style="margin:24px 0 8px 0;font-size:13px;color:${COLORS.inkMuted}">
          <b>Contact direct :</b> ${requesterContact}
        </p>` : ''}

        <p style="margin:24px 0 8px 0;font-size:13px;color:${COLORS.inkLight};line-height:1.5">
          Tu peux <b>répondre directement à cet email</b> (le Reply-To pointe sur ${requesterOrgName}) ou ignorer si la mise en relation ne t'intéresse pas. Aucune obligation.
        </p>

        ${cta(oppUrl, "Revoir l'opportunité")}

        <p style="margin:32px 0 0 0;font-size:12px;color:${COLORS.inkLight};line-height:1.5">
          💡 Pourquoi tu reçois ça : tu as opté in à la co-soumission sur cette opp depuis ton dashboard. Tu peux retirer ce signal à tout moment depuis la page de l'opp.
        </p>
      `,
    })
  };
